...
4) [контроллер ProxyController](#контроллер-proxycontroller)
5) [контроллер SecurityController](#контроллер-securitycontroller) 
6) [класс UserCache](#класс-usercache)
7) [Модели данных](#модели-данных)
8) [Конфигурация безопасности SecurityConfig](#конфигурация-безопасности-securityconfig)
9) [Обработчик глобальных исключений](#обработчик-глобальных-исключений)
10) [Репозитории Spring Data JPA](#репозитории-spring-data-jpa)
11) [Классы для JSON сериализации](#классы-для-json-сериализации)
12) [Сервисы](#сервисы)
13) [Сервис AlbumService](#интерфейс-albumservice)
14) [Сервис AuditLogService](#интерфейс-AuditLogService)
15) [Сервис PostService](#интерфейс-PostService)
16) [Сервис SecurityService](#интерфейс-SecurityService)
17) [Сервис UserService](#интерфейс-UserService)
18) [Фаил application.properties](#properties)
19) [Сборка Maven](#maven)

# О проекте

Проект представляет собой REST API, реализованное на Java с использованием фреймворка Spring. Он служит в качестве прокси-сервера для запросов к внешнему веб-сервису, доступному по адресу https://jsonplaceholder.typicode.com/. Прокси-сервер принимает запросы от клиентов и перенаправляет их на соответствующие обработчики внешнего веб-сервиса.

Проект включает в себя базовую авторизацию, реализованную с помощью Spring Security, а также проработанную ролевую модель доступа к данным. Роли определены следующим образом: ROLE_ADMIN имеет доступ ко всем обработчикам, ROLE_POSTS, ROLE_USERS и ROLE_ALBUMS имеют доступ к соответствующим разделам обработчиков.

Кроме того, в проекте реализовано ведение аудита действий, включая информацию о дате-времени, пользователе, параметрах запроса и т. д. В целях оптимизации производительности использован in-memory кэш для уменьшения числа запросов к внешнему веб-сервису.

Кратко говоря, проект представляет собой промежуточное звено между клиентами и внешним веб-сервисом, обеспечивая безопасность, управление доступом и ведение аудита запросов.
